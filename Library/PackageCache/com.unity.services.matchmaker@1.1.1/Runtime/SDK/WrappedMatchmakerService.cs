...
namespace Unity.Services.Matchmaker
{
    internal class WrappedMatchmakerService : IMatchmakerSdkConfiguration, IMatchmakerService
    {
        ...
        internal WrappedMatchmakerService(IMatchmakerServiceSdk matchmakerService, IPayloadProxyApiClient payloadProxyClient = null, IABRemoteConfig abRemoteConfig = null, IABAnalytics abAnalytics = null)
        {
            ...
            {
                var installationId = CoreRegistry.Instance.GetServiceComponent<IInstallationId>();
                var userId = installationId?.GetOrCreateIdentifier();
                var projectConfiguration = CoreRegistry.Instance.GetServiceComponent<IProjectConfiguration>();
                var cloudEnvironment = projectConfiguration?.GetString(CloudEnvironmentKey);

                m_abRemoteConfig = new ABRemoteConfig(new HttpClient(), userId, cloudEnvironment);
            }
            else
            {
            ...
            if (abAnalytics == null)
            {
                var environmentId = CoreRegistry.Instance.GetServiceComponent<IEnvironmentId>()?.EnvironmentId;
                m_abAnalytics = new ABAnalytics(environmentId);
            }
            else
            {
        ...
        public async Task<CreateTicketResponse> CreateTicketAsync(List<Player> players, CreateTicketOptions options)
        {
            ...
            var model = new CreateTicketRequest(
                ...
                queueName,
                attributes);
            // Only bother with overrides if the analytics package is installed.
