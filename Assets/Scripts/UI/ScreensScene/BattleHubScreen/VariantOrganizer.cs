    ...
    public class VariantOrganizer
    {
        ...
        public List<int> GetOrganizedVariants()
        {
            try
            {
                ...
                {
                    Debug.LogWarning("[VariantOrganizer] No variants found");
                    return new List<int>();
                }
                ...
                Debug.Log($"[VariantOrganizer] Found {allVariants.Count} total variants");
                
                var uncachedVariants = allVariants.Where(v => !_variantParentCache.ContainsKey(v.Index)).ToList();
                if (uncachedVariants.Any())
                {
                    Debug.LogWarning($"[VariantOrganizer] Found {uncachedVariants.Count} variants without parents");
                }
                ...
                sb.AppendLine($"[VariantOrganizer] Returning {result.Count} organized variants:");
                ...
                {
                    Debug.LogWarning($"[VariantOrganizer] Lost {allVariants.Count - uncachedVariants.Count - result.Count} variants during organization");
                }
            ...
            {
                Debug.LogError($"[VariantOrganizer] Critical error: {e}");
            ...
            lock (_cacheLock)
            {
                ...
                foreach (var variant in variants)
                {
                    try
                    {
                        ...
                        if (prefab == null)
                        {
                            Debug.LogWarning($"[VariantOrganizer] Null prefab for variant {variant.Index}");
                            continue;
                        }

                        if (prefab.parent == null)
                        {
                            Debug.LogWarning($"[VariantOrganizer] Null parent for variant {variant.Index}");
                            continue;
                        }
                    ...
                    {
                        Debug.LogError($"[VariantOrganizer] Failed to cache variant {variant.Index}: {e}");
                    }
                }

                Debug.Log($"[VariantOrganizer] Cached {_variantParentCache.Count}/{variants.Count} variant parents");
            }
        }
            ...
            IReadOnlyList<VariantData> allVariants)
        {
            ...
            try 
            {
                ...
                #if UNITY_EDITOR || DEVELOPMENT_BUILD
                Debug.Log($"[VariantOrganizer] Processed variants by category:" +
                ...
                {
                    Debug.LogWarning($"[VariantOrganizer] Variant count mismatch! " +
                                   $"Processed {processedVariants.Count} vs Input {allVariants.Count}");
                }
            }
            catch (System.Exception e)
            {
                Debug.LogError($"[VariantOrganizer] Error categorizing variants: {e}");
            }
            ...
            where T : IPrefabKey
        {
            ...
            {
                Debug.LogError($"[VariantOrganizer] Error reordering variants: {e}");
        ...
        private List<int> SortVariantGroup(IGrouping<string, VariantData> group)
        {
            ...
            {
                Debug.LogError($"[VariantOrganizer] Error sorting variant group: {e}");
                return new List<int>();
            }
        }
        ...
        {
            Debug.Log($"[VariantOrganizer] Example variant {variant.Index}:" +
