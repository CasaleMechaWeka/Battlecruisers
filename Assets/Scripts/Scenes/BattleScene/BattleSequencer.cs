...
namespace BattleCruisers.Scenes.BattleScene
{
    public class BattleSequencer : MonoBehaviour
    {
                    ...
                    switch (buildingAction.Operation)
                    {
                        ...
                        case SequencePoint.BuildingAction.BuildingOp.Add:
                             ...
                             && (prefabID < (int)Unit_Bomber))
                            {
                                ...
                                // NOTE: Cruiser.ConstructBuilding signature is (ignoreBuildTime, ignoreDroneRequirement)
                                // These were previously passed in the wrong order, causing sequencer flags to behave incorrectly.
                                cruiser.ConstructBuilding(
                                    building,
                                    slot,
                                    ignoreBuildTime: buildingAction.IgnoreBuildTime,
                                    ignoreDroneRequirement: buildingAction.IgnoreDroneReq);
                            }
                            else
                            {
            ...
            if (sq.UnitActions != null)
                foreach (SequencePoint.UnitAction unitAction in sq.UnitActions)
                {
                    ...
                    if (unitAction.Amount == 1)
                        SpawnUnit(unitAction.PrefabKeyName, unitAction.Postion, cruiser, unitAction.ApplyNavalYSpawnOffset);
                    else
                    {
                        for (int i = 0; i < unitAction.Amount; i++)
                        {
                            ...
                            SpawnUnit(unitAction.PrefabKeyName, pos, cruiser, unitAction.ApplyNavalYSpawnOffset);
                        }
                    }
                }
        ...
        /// <summary>
        /// Spawn a unit at a given world position, with an optional factory-like naval Y offset.
        /// Note: naval factories apply <see cref="IUnit.YSpawnOffset"/> (see <c>NavalFactorySpawnPositionFinder</c>).
        /// Sequencer spawns historically used raw world coordinates, so this remains opt-in.
        /// </summary>
        public void SpawnUnit(PrefabKeyName prefabKey, Vector2 position, Cruiser cruiser, bool applyNavalYSpawnOffset)
        {
            IBuildableWrapper<IUnit> unitWrapper = PrefabFactory.GetUnitWrapperPrefab(StaticPrefabKeyHelper.GetPrefabKey<UnitKey>(prefabKey));
            IUnit unit = PrefabFactory.CreateUnit(unitWrapper);

            if (applyNavalYSpawnOffset)
            {
                position.y += unit.YSpawnOffset;
            }

            unit.Transform.Position = new Vector3(position.x, position.y, 0);
            
            BuildableActivationArgs buildableActivationArgs = new BuildableActivationArgs(
                cruiser,
                cruiser.CruiserSpecificFactories.EnemyCruiser,
                cruiser.CruiserSpecificFactories);
            unit.Activate(buildableActivationArgs);
    ...
    public class SequencePoint
    {
        ...
        public class UnitAction
        {
            ...
            public bool ApplyNavalYSpawnOffset = false;
            ...
            public override string ToString()
            {
                string s = "UnitAction: {\n +"
                 ...
                 + $"\tArea: {SpawnArea}\n"
                 + $"\tAmount: {Amount}\n"
                 + $"\tApplyNavalYSpawnOffset: {ApplyNavalYSpawnOffset}\n"
                 + "}\n";
                return s;
            }
        }
